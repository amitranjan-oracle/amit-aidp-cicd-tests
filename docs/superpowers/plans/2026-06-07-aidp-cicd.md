...
| Path | Responsibility |
|---|---|
| `deploy/aidp_deploy.py` | Orchestrator + thin AIDP client. Pure helpers (config/spec load, normalize, subset-diff, key-inject), `AidpClient` (signed REST), phases 1–4, `main`. |
| `deploy/cicd.yaml` | Identifiers, spec-file paths, `git_credential_key` reference. |
| `specs/cicd_01.cluster.json` | Desired cluster definition. |
| `specs/cicd_workflow_job.job.json` | Desired job definition (cluster referenced by name). |
| `deploy/requirements-cicd.txt` | `oci`, `requests`, `pyyaml` (deps already on the box; documentation + preflight). |
...
| `tests/test_deploy/aidp_deploy.py` | `unittest` tests for the pure helpers. |
...
**Files:**
- Create: `deploy/cicd.yaml`
- Create: `specs/cicd_01.cluster.json`
- Create: `specs/cicd_workflow_job.job.json`
- Create: `deploy/requirements-cicd.txt`

- [ ] **Step 1: Create `deploy/cicd.yaml`**
...
- [ ] **Step 2: Create `specs/cicd_01.cluster.json`**
...
```json
{
  ...
  "jobClusters": [ { "clusterName": "cicd_01", "newCluster": null } ],
  "tasks": [
    { "type": "NOTEBOOK_TASK", "taskKey": "set_parameter", "dependsOn": [],
      ...
      "cluster": { "clusterName": "cicd_01", "newCluster": null },
      "parameters": [] },
...
- [ ] **Step 4: Create `deploy/requirements-cicd.txt`**
...
```bash
git add deploy/cicd.yaml specs/cicd_01.cluster.json specs/cicd_workflow_job.job.json deploy/requirements-cicd.txt
...
**Files:**
- Create: `deploy/aidp_deploy.py`
- Create: `tests/test_deploy/aidp_deploy.py`
...
class TestConfigLoading(unittest.TestCase):
    ...
    def test_load_config_ok(self):
        ...
        cfg = aidp_deploy.load_config(path)
        self.assertEqual(cfg["aidp"]["region"], "us-ashburn-1")
        self.assertEqual(cfg["workflow"]["cluster_name"], "cicd_01")

    def test_load_config_missing_keys(self):
        path = self._write("aidp:\n  region: us-ashburn-1\n")
        with self.assertRaises(ValueError) as ctx:
            aidp_deploy.load_config(path)
        self.assertIn("git.repository_url", str(ctx.exception))

    def test_load_spec(self):
        fd, path = tempfile.mkstemp(suffix=".json")
        with os.fdopen(fd, "w") as f:
            json.dump({"displayName": "cicd_01"}, f)
        self.addCleanup(os.remove, path)
        self.assertEqual(aidp_deploy.load_spec(path)["displayName"], "cicd_01")
...
Expected: FAIL — `ModuleNotFoundError: No module named 'aidp_deploy'` (or `AttributeError: load_config`).

- [ ] **Step 3: Create `deploy/aidp_deploy.py` with the module header and loaders**
...
```bash
git add deploy/aidp_deploy.py tests/test_deploy/aidp_deploy.py
...
**Files:**
- Modify: `deploy/aidp_deploy.py`
- Modify: `tests/test_deploy/aidp_deploy.py`

- [ ] **Step 1: Write the failing tests** (append to `tests/test_deploy/aidp_deploy.py`)
...
class TestSubsetDiff(unittest.TestCase):
    def test_satisfies_scalar_and_nested(self):
        ...
        self.assertTrue(aidp_deploy.satisfies(desired, live))   # extras ignored

    def test_satisfies_detects_change(self):
        self.assertFalse(aidp_deploy.satisfies({"a": 1}, {"a": 2}))
        self.assertFalse(aidp_deploy.satisfies({"a": 1}, {}))           # missing key

    def test_satisfies_lists_elementwise_subset(self):
        desired = {"t": [{"k": "x"}]}
        live = {"t": [{"k": "x", "extra": 1}]}
        self.assertTrue(aidp_deploy.satisfies(desired, live))
        self.assertFalse(aidp_deploy.satisfies({"t": [{"k": "x"}]}, {"t": []}))

    def test_cluster_satisfied_ignores_volatile(self):
        desired = {"displayName": "cicd_01",
                   "driverConfig": {"driverShape": "amd.generic"}}
        live = {"displayName": "cicd_01", "key": "abc", "state": "ACTIVE",
                "driverConfig": {"driverShape": "amd.generic", "driverNodeType": None},
                "jdbcEndpointUrl": "jdbc:..."}
        self.assertTrue(aidp_deploy.cluster_in_sync(desired, live))

    def test_job_compares_cluster_by_name_not_key(self):
        desired = {"name": "cicd_workflow_job",
                   "jobClusters": [{"clusterName": "cicd_01"}]}
        live = {"name": "cicd_workflow_job", "key": "j1",
                "jobClusters": [{"clusterName": "cicd_01",
                                 "clusterKey": "a3ad", "newCluster": None}]}
        self.assertTrue(aidp_deploy.job_in_sync(desired, live))
```

- [ ] **Step 2: Run tests to verify they fail**

Run: `/opt/anaconda3/bin/python3 -m unittest discover -s tests -v`
Expected: FAIL — `AttributeError: module 'aidp_deploy' has no attribute 'satisfies'`.

- [ ] **Step 3: Implement subset diff** (append to `deploy/aidp_deploy.py`)
...
- [ ] **Step 1: Write the failing test** (append to `tests/test_deploy/aidp_deploy.py`)

```python
class TestInjectClusterKey(unittest.TestCase):
    def test_injects_into_jobclusters_and_tasks(self):
        job = {
            "jobClusters": [{"clusterName": "cicd_01", "newCluster": None}],
            "tasks": [
                {"taskKey": "a", "cluster": {"clusterName": "cicd_01"}},
                {"taskKey": "b", "cluster": {"clusterName": "cicd_01"}},
            ],
        }
        out = aidp_deploy.inject_cluster_key(job, "KEY-123")
...
- [ ] **Step 3: Implement** (append to `deploy/aidp_deploy.py`)
...
class TestUrlBuilding(unittest.TestCase):
    def test_lake_and_ws_urls(self):
        c = aidp_deploy.AidpClient.__new__(aidp_deploy.AidpClient)
...
- [ ] **Step 3: Implement the signer + client request layer** (append to `deploy/aidp_deploy.py`)
...
- [ ] **Step 1: Add directory + git-folder + async methods to `AidpClient`** (append inside the class in `deploy/aidp_deploy.py`)
...
Run: `/opt/anaconda3/bin/python3 -m py_compile deploy/aidp_deploy.py`
...
- [ ] **Step 1: Add phase functions** (append at module level in `deploy/aidp_deploy.py`)
...
Run: `/opt/anaconda3/bin/python3 -m py_compile deploy/aidp_deploy.py && /opt/anaconda3/bin/python3 -m unittest discover -s tests -v`
Expected: compile clean; 10 tests PASS.

- [ ] **Step 4: Local dry-run smoke (config parsing + phase wiring)**

Run: `/opt/anaconda3/bin/python3 deploy/aidp_deploy.py --config deploy/cicd.yaml --dry-run`
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Reconcile AIDP
        run: |
          python3 deploy/aidp_deploy.py --config deploy/cicd.yaml
```
...
self-hosted runner** (Linux x64), or `gh api -X POST repos/amitranjan-oracle/amit-aidp-cicd-tests/actions/runners/registration-token`.
...
Run: `python3 deploy/aidp_deploy.py --config deploy/cicd.yaml --dry-run`
...
Run: `gh workflow run aidp-cicd --repo amitranjan-oracle/amit-aidp-cicd-tests --ref main`
Then watch: `gh run watch --repo amitranjan-oracle/amit-aidp-cicd-tests`
Expected: job picked up by the self-hosted runner; all steps green.

- [ ] **Step 3: Verify results via AIDP MCP**

- `list_files` at `/Workspace/cicd_folder/amit-aidp-cicd-tests` → repo content present.
- Resolve `cicd_workflow_job` (`aidp_roles`/`get_job` or `list_jobs`) → 2 sequential tasks `set_parameter → print_parameter`, both on cluster `cicd_01`.
- `get_cluster_status` for `cicd_01` → matches the spec.

Expected: all present and correct.

- [ ] **Step 4: Re-run to prove idempotency**

Run: `gh workflow run aidp-cicd --repo amitranjan-oracle/amit-aidp-cicd-tests --ref main && gh run watch --repo amitranjan-oracle/amit-aidp-cicd-tests`
Expected logs: Phase 2 `pulling main`; Phase 3 `cluster cicd_01 already in sync -> NO-OP`; Phase 4 `job cicd_workflow_job already in sync -> NO-OP`. No duplicate resources.
...
- Two JSON specs (job + `cicd_01`) → Task 1. ✅
- Reconcile create/update/no-op for compute + job → Task 3 (diff), Task 7 (phases 3–4). ✅
- Job cloned from `7fa6a6bf`, repointed to `cicd_01`, key injected → Task 1 (spec), Task 4 (inject), Task 7 Phase 4. ✅
